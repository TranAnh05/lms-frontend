...
export const PermissionPage: React.FC = () => {
    ...
    // Tải toàn bộ cấu hình dữ liệu phân quyền
    const fetchPageData = useCallback(async () => {
        ...
        try {
            ...
            // Tạo bản đồ ánh xạ nhanh từ Code sang ID
            const permCodeToIdMap = new Map<string, number>();
            permsData.forEach((p) => permCodeToIdMap.set(p.code, p.id));

            // Xây dựng cấu trúc ma trận phân quyền ban đầu
    ...
    // Xử lý sự kiện thay đổi trạng thái ô checkbox
    ...
    // Lưu cấu hình phân quyền của một vai trò cụ thể xuống cơ sở dữ liệu
    const handleSaveRole = useCallback(
        async (roleId: number) => {
            ...
            try {
                ...
                const roleName = roles.find((r) => r.id === roleId)?.name || `ID: ${roleId}`;
                toast.success(`Đã cập nhật quyền hạn cho vai trò: ${roleName}`);
            } catch (error) {
                const apiError = error as ApiError;
                const errorMsg = apiError.response?.data?.message || "Cập nhật cấu hình thất bại. Vui lòng thử lại.";
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 tracking-tight flex items-center gap-2">
                        <ShieldCheck className="w-7 h-7 text-blue-600" /> Quản lý Phân quyền
                    </h1>
                    <p className="text-sm text-gray-500 mt-1">
                        Thiết lập ma trận chức năng cho các nhóm vai trò trong hệ thống.
                    </p>
                </div>
                ...
                <button
                    ...
                    className="self-start sm:self-auto flex items-center gap-2 px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 hover:text-blue-600 transition-colors shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 disabled:opacity-50 disabled:pointer-events-none"
                >
            ...
            <div className="bg-blue-50/80 border border-blue-100 rounded-xl p-4 flex gap-3 items-start shadow-sm">
                ...
                <div className="text-sm text-blue-800 leading-relaxed">
                    ...
                    <ul className="list-disc list-inside space-y-1 text-blue-700/90 ml-1">
                        <li>
                            Tích chọn hoặc bỏ chọn các ô vuông để gán hoặc thu hồi quyền của một vai trò tương ứng.
                        </li>
                        <li>
                        ...
                        <li>
                            Các quyền được tự động gom nhóm theo Module để dễ dàng quản lý.
                        </li>
                    </ul>
                </div>
            </div>
