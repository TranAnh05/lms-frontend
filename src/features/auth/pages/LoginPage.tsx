    ...
    <div className="hidden md:flex md:w-1/2 bg-blue-600 p-12 flex-col justify-between relative overflow-hidden">
        ...
        <div className="relative z-10">
            ...
            <p className="text-blue-100 text-lg mb-8 max-w-sm">
                Tối ưu hóa quy trình giảng dạy, quản lý học vụ và kết nối sinh
                viên trên một hệ thống duy nhất.
            </p>
...
export const LoginPage: React.FC = () => {
    ...
    // Toi uu 1: Tach biet selector xoa bo hoan toan vong lap render vo han
    const loginSuccess = useAuthStore((state) => state.loginSuccess);
    const logout = useAuthStore((state) => state.logout);

    const [isLoading, setIsLoading] = useState(false);

    // Toi uu 2: Boc useCallback on dinh tham chieu ham truyen xuong Form component
    const handleLoginSubmit = useCallback(
        async (data: LoginFormData) => {
            try {
                setIsLoading(true);

                // Clear session cu truoc khi dang nhap
                logout();
