...
// Sử dụng as const để đóng băng cấu hình, tối ưu bộ nhớ và cải thiện kiểm tra kiểu của TypeScript
const STATUS_CONFIG = {
    REGISTERED: {
        label: "Ghi nhận",
            ...
            return (
                <div className="flex flex-col items-center justify-center py-20 bg-white rounded-xl border border-gray-100 shadow-sm text-center">
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                        Bạn chưa đăng ký lớp học phần nào. Hãy chọn lớp từ danh
                        sách phía trên.
                    </p>
                </div>
            );
                    ...
                    <table className="w-full text-sm text-left">
                        <thead className="text-xs text-gray-500 uppercase bg-gray-50/50 border-b border-gray-100">
                            <tr>
                                ...
                                <th className="px-5 py-3 font-semibold min-w-[200px]">
                                    Tên môn học
                                </th>
                                <th className="px-5 py-3 font-semibold">
                                    Mã lớp
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {data.map((item) => {
                                // Ép kiểu an toàn để đảm bảo không lỗi compiler khi đối chiếu trạng thái từ API
                                const currentStatusKey =
                                    item.status as StatusKey;
                                const status =
                                    STATUS_CONFIG[currentStatusKey] ||
                                ...
                                const dayStr =
                                    item.dayOfWeek === 8
                                        ? "CN"
                                        : `T${item.dayOfWeek}`;
                                const timeString = item.dayOfWeek
                                    ? `${dayStr} (${item.shiftName})`
                                    : "";

                                return (
                                    <tr
                                        key={`${item.classId}-${item.enrolledAt}`}
                                        className="hover:bg-gray-50/50 bg-white transition-colors"
                                    >
...
RegisteredClassesTable.displayName = "RegisteredClassesTable";
