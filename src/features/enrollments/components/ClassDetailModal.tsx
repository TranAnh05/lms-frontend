...
export const ClassDetailModal: React.FC<ClassDetailModalProps> = React.memo(
    ({ isOpen, onClose, classData, courseName, courseCode }) => {
        // Neu modal khong mo hoac thieu du lieu thi khong render
        if (!isOpen || !classData) return null;

        // Boc tach thuoc tinh de code ngan gon va sach se hon
        const {
            classCode,
            currentStudents,
            maxStudents,
        ...
        return (
            <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm animate-in fade-in duration-200">
                {/* Backdrop vung ngoai de dong modal khi click */}
                            ...
                            {!dayOfWeek || !shiftName ? (
                                <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 flex items-start gap-3 text-amber-800">
                                    ...
                                    <p className="text-sm font-medium">
                                        Lớp học phần này hiện chưa có lịch học
                                        cụ thể. Vui lòng quay lại kiểm tra sau.
                                    </p>
                                </div>
                            ) : (
