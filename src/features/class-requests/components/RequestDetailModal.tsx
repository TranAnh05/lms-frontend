...
// Cấu hình cố định giao diện Badge trạng thái ngoài component
const STATUS_BADGE_MAP: Record<ClassOpeningResponseDto["status"], React.ReactNode> = {
    PENDING: (
...
export const RequestDetailModal: React.FC<RequestDetailModalProps> = ({
    ...
    // Đóng nhanh nếu trạng thái ẩn hoặc không có dữ liệu
            ...
            <div className="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl flex flex-col animate-in zoom-in-95 duration-200">
                ...
                <div className="p-6 flex flex-col gap-6 max-h-[75vh] overflow-y-auto custom-scrollbar">
                    ...
                    <div className="flex items-center justify-between pb-4 border-b border-gray-100">
                        <div>
                            ...
                            {STATUS_BADGE_MAP[request.status] || null}
                        </div>
                    ...
                    {/* Khu vực hiển thị Lý do từ chối */}
                ...
                <div className="p-5 border-t border-gray-100 bg-gray-50/80 rounded-b-2xl flex justify-end gap-3">
                    <button 
                        type="button" 
                        onClick={onClose} 
                        className="px-5 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors focus:outline-none"
                    >
                        Đóng
                    </button>
                    ...
                    {request.status === "PENDING" && canApprove && (
                        <>
                            <button 
                                type="button" 
                                onClick={() => { onClose(); onRejectClick(request); }} 
                                className="px-5 py-2.5 text-sm font-medium text-rose-600 bg-rose-50 border border-rose-200 rounded-lg hover:bg-rose-100 transition-colors focus:outline-none"
                            >
                                Từ chối
                            </button>
                            <button 
                                type="button" 
                                onClick={() => { onClose(); onCreateClassClick(request); }} 
                                className="inline-flex items-center justify-center gap-1.5 px-5 py-2.5 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors focus:outline-none"
                            >
                                <Layers className="w-4 h-4" /> Phê duyệt & Tạo lớp
                            </button>
                        </>
                    )}
                </div>
