...
// Định nghĩa cấu trúc lỗi từ API để ép kiểu an toàn thay vì dùng any
interface ApiError {
    response?: {
        data?: {
            message?: string;
...
interface CreateClassFromRequestModalProps {
    ...
    onConfirm: (payload: CreateClassPayload) => Promise<void>;
}

export const CreateClassFromRequestModal: React.FC<
    CreateClassFromRequestModalProps
> = ({ isOpen, onSuccess, request, onConfirm }) => {
    ...
    const [isLoadingDropdowns, setIsLoadingDropdowns] =
        useState<boolean>(false);

    // Tải danh mục dropdown phòng và ca học khi mở modal
    useEffect(() => {
        if (!isOpen) return;
    ...
    // Khởi tạo cấu hình mặc định (1 lớp) khi modal được mở lần đầu
    useEffect(() => {
        if (isOpen && request) {
            setClassesData([
                { dayOfWeek: 2, shiftId: "", roomId: "", maxStudents: 40 },
            ]);
        } else {
            setClassesData([]);
        }
    ...
    // Thay đổi số lượng phân lớp
    const handleClassCountChange = useCallback((count: number) => {
    ...
    // Cập nhật từng trường thông tin trong danh sách lớp
    const handleClassChange = useCallback(
        (index: number, field: keyof ClassConfigItem, value: number | "") => {
            setClassesData((prev) => {
                const temp = [...prev];
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        for (let i = 0; i < classesData.length; i++) {
            ...
            if (!cls.roomId || !cls.shiftId) {
                toast.error(
                    `Vui lòng chọn Phòng và Ca học cho Lớp thứ ${i + 1}.`,
                );
                return;
            }
        ...
        try {
            ...
            const errorMessage =
                ...
                "Đã xảy ra lỗi khi khởi tạo lớp học phần. Vui lòng kiểm tra lại.";
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-4 border-t border-gray-100">
                                <div>
                                    <h4 className="text-sm font-bold text-gray-800">
                                        Số lượng phân lớp thực tế
                                    </h4>
                                    <p className="text-xs text-gray-500 mt-1">
                                        Hệ thống sẽ tự động sinh mã lớp học phần
                                        (Ví dụ: SWE101-01, SWE101-02).
                                    </p>
                                </div>
