...
export const MajorFormModal: React.FC<MajorFormModalProps> = ({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editId) {
                await majorService.updateMajor(editId, {
                    name: formData.name.trim(),
                    requiredMinimumCredits: formData.requiredMinimumCredits,
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 mt-0.5">
                            {isEditMode ? "Chỉnh sửa thông tin cơ bản của ngành đào tạo" : "Khởi tạo ngành đào tạo mới trong hệ thống"}
                        </p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden relative">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                            <div className="sm:col-span-1">
                                ...
                                <input
                                    ...
                                    placeholder="VD: PM"
                                    className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none uppercase font-mono tracking-wider disabled:opacity-70 disabled:cursor-not-allowed"
                                />
                            </div>
                    ...
                    <div className="px-6 py-4 bg-gray-50/50 border-t border-gray-100 flex items-center justify-end gap-3 shrink-0">
                        ...
                        <button
                            type="submit"
                            disabled={isSubmitting || isFetching}
                            className="flex items-center justify-center gap-2 px-6 py-2.5 text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-xl transition-colors shadow-sm focus:outline-none disabled:opacity-70 disabled:cursor-not-allowed min-w-[140px]"
                        >
                            {isSubmitting ? (
