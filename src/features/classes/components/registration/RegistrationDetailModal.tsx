...
// Toi uu: Khoi tao bo dinh dang thoi gian 1 lan duy nhat ngoai Component
...
> = ({ isOpen, onClose, periodId, onSuccess }) => {
    const [detailData, setDetailData] =
        useState<RegistrationPeriodDetailResponse | null>(null);
    ...
    useEffect(() => {
        ...
        // Toi uu: Su dung AbortController de chan memory leak khi dong Modal som
        ...
        const fetchDetail = async () => {
            ...
            try {
                const response =
                    await registrationService.getRegistrationPeriodDetail(
                        periodId,
                    );
                ...
                if (!abortController.signal.aborted) {
                    console.error(error);
                    toast.error(
                        "Không thể tải thông tin chi tiết đợt đăng ký.",
                    );
                    onClose();
                }
            } finally {
        ...
        return () => {
            ...
            setDetailData(null); // Don dep du lieu cu
        };
    }, [periodId, isOpen, onClose]);

    // Toi uu: Boc useCallback de giu tham chieu ham on dinh
    const handleForceClose = useCallback(async () => {
        if (!periodId) return;

        const confirmMsg =
            "Bạn có chắc chắn muốn đóng đợt đăng ký này sớm hơn dự kiến? Hành động này sẽ chuyển tất cả các lớp đang mở sang trạng thái 'Đang diễn ra'.";
                                ...
                                <div className="flex items-center gap-3 mb-2">
                                    ...
                                    <span
                                        className={clsx(
                                            ...
                                            STATUS_UI_CONFIG[detailData.status]
                                                ?.style,
                                        )}
                                    >
                                        {
                            ...
                            <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden flex flex-col">
                                <div className="px-5 py-4 border-b border-gray-100 bg-gray-50/50">
                                    <h4 className="font-bold text-gray-900 text-sm tracking-tight">
                                        Danh sách Lớp học phần trong đợt
                                    </h4>
                                </div>
                                    ...
                                    <table className="w-full text-sm text-left text-gray-500 border-collapse min-w-[800px]">
                                        <thead className="text-xs text-gray-400 uppercase bg-white border-b border-gray-100 font-bold tracking-wider whitespace-nowrap">
                                            <tr>
                                                ...
                                                <th
                                                    scope="col"
                                                    className="px-5 py-4 min-w-[250px]"
                                                >
                                                    Môn học
                                                </th>
                                                <th
                                            ...
                                            {detailData.classes.length === 0 ? (
                                                <tr>
                                                    ...
                                                    >
                                                        ...
                                                        được gắn vào đợt này.
                                                    </td>
                                                </tr>
                                            ) : (
                                                detailData.classes.map(
                                                    (cls) => {
                                                        const percent =
                                                            Math.min(
                                                                Math.round(
