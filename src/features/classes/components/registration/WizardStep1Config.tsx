...
const AVAILABLE_COHORTS = [14, 15, 16, 17, 18];
...
export const WizardStep1Config: React.FC<WizardStep1ConfigProps> = ({
    ...
    const isFormValid = useMemo(() => {
        const hasBasicInfo =
            !!formData.name.trim() && !!formData.semesterId && !!formData.type;
        ...
        <div className="flex flex-col h-full animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="mb-6">
                ...
                <p className="text-sm text-gray-500 mt-1">
                    Thiết lập thông tin chung, thời gian và đối tượng sinh viên
                    được phép tham gia đăng ký học phần.
                </p>
            </div>
            ...
            <div className="flex-1 space-y-6 overflow-y-auto custom-scrollbar pr-2 pb-6">
                ...
                <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm space-y-4">
                    <div>
                        <label className={labelClasses}>
                            <Type className="w-4 h-4 text-gray-400" /> Tên đợt
                            đăng ký <span className="text-rose-500">*</span>
                        </label>
                        <input
                            type="text"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className={labelClasses}>
                                <Tag className="w-4 h-4 text-gray-400" /> Loại
                                đợt <span className="text-rose-500">*</span>
                            </label>
                            <select
                                value={formData.type}
                                onChange={(e) =>
                ...
                <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className={labelClasses}>
                                <Clock className="w-4 h-4 text-gray-400" /> Thời
                                gian mở <span className="text-rose-500">*</span>
                            </label>
                            <input
                                type="datetime-local"
                    ...
                    {isTimeInvalid && (
                        <div className="mt-3 flex items-center gap-2 text-sm text-rose-600 bg-rose-50 px-3 py-2 rounded-lg border border-rose-100">
                ...
                <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm space-y-5">
                    <div>
                        <label className={labelClasses}>
                            <Building2 className="w-4 h-4 text-gray-400" /> Khoa
                            được phép đăng ký{" "}
                            <span className="text-rose-500">*</span>
                        </label>
                        <div className="flex flex-wrap gap-2 mt-2">
                            <button
                                ...
                                onClick={() =>
                                    onChange({
                                        targetDepartments: departments.map(
                                            (d) => d.id,
                                        ),
                                    })
                                }
                                className="text-xs font-medium text-blue-600 hover:text-blue-800 underline underline-offset-2 mr-2 mb-2 focus:outline-none"
                            >
                                Chọn tất cả
                            </button>
                            {departments.map((dept) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={clsx(
                                            "px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 border focus:outline-none focus:ring-2 focus:ring-offset-1",
                                            isSelected
                                                ? "bg-blue-50 border-blue-300 text-blue-700 ring-1 ring-blue-500 focus:ring-blue-500"
                                                : "bg-white border-gray-200 text-gray-600 hover:bg-gray-50 hover:border-gray-300 focus:ring-gray-200",
                                        )}
                                    >
                                        {dept.name}
                                    </button>
                            ...
                            {AVAILABLE_COHORTS.map((cohort) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={clsx(
                                            "px-5 py-2 rounded-lg text-sm font-bold transition-all duration-200 border focus:outline-none focus:ring-2 focus:ring-offset-1",
                                            isSelected
                                                ? "bg-blue-600 border-blue-600 text-white shadow-sm focus:ring-blue-500"
                                                : "bg-white border-gray-200 text-gray-600 hover:bg-gray-50 hover:border-gray-300 focus:ring-gray-200",
                                        )}
                                    >
                                        Khóa {cohort}
