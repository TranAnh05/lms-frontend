...
// Toi uu: Khoi tao bo dinh dang thoi gian 1 lan duy nhat de tiet kiem tai nguyen CPU
...
// Toi uu: Dua mang tinh ra ngoai vong doi render
const SKELETON_ROWS = Array(5).fill(0);

// Toi uu: Su dung memo de ngan chan re-render khi component cha thay doi state khong lien quan
export const RegistrationTable: React.FC<RegistrationTableProps> = memo(({
    data,
    isLoading,
    onPageChange,
    onViewDetail,
                ...
                <table className="w-full text-sm text-left text-gray-500 table-auto min-w-[900px]">
                    <thead className="text-xs text-gray-700 uppercase bg-gray-50/80 border-b border-gray-100 font-bold">
                        <tr>
                            ...
                            <th scope="col" className="px-5 py-4 w-[20%] whitespace-nowrap">
                                Thời gian áp dụng
                            </th>
                            <th scope="col" className="px-5 py-4 w-[10%] text-center whitespace-nowrap">
                                Trạng thái
                            </th>
                            <th scope="col" className="px-3 py-4 text-center w-[10%] sticky right-0 bg-gray-50/80 shadow-[-4px_0_6px_-2px_rgba(0,0,0,0.02)] z-10 whitespace-nowrap">
                                Thao tác
                            </th>
                        </tr>
                    </thead>
                        ...
                        {isLoading ? (
                            SKELETON_ROWS.map((_, index) => (
                                <tr key={index} className="animate-pulse bg-white">
                                    ...
                                    <td className="px-5 py-4 sticky right-0 bg-white"><div className="h-8 bg-gray-200 rounded-lg w-20 mx-auto"></div></td>
                                </tr>
                            ))
                                ...
                                <td colSpan={6} className="px-6 py-16 text-center bg-white">
                                    <div className="flex flex-col items-center justify-center gap-2 text-gray-400">
                                        ...
                                        <span className="text-sm">
                                            Vui lòng thay đổi bộ lọc hoặc tạo mới đợt đăng ký.
                                        </span>
                                    </div>
                                </td>
                            </tr>
                            ...
                            data.content.map((period) => {
                                ...
                                return (
                                    <tr key={period.id} className="hover:bg-blue-50/40 bg-white transition-colors group">
                                        <td className="px-5 py-4">
                                            <p className="font-bold text-gray-900 leading-tight truncate max-w-[280px]" title={period.name}>
                                                {period.name}
                                            </p>
                                        </td>
                                        ...
                                        <td className="px-5 py-4 whitespace-nowrap">
                                            <div className="flex flex-col gap-1.5 text-xs text-gray-600 font-medium">
                                                <span className="flex items-center gap-1.5">
                                                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> Mở: {formatDateTime(period.startTime)}
                                                </span>
                                                <span className="flex items-center gap-1.5">
                                                    <span className="w-1.5 h-1.5 rounded-full bg-rose-500"></span> Đóng: {formatDateTime(period.endTime)}
                                                </span>
                                            </div>
                                        </td>
                                        <td className="px-5 py-4 text-center whitespace-nowrap">
                                            <span className={clsx("inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold tracking-wide border", statusUI.style)}>
                                                {statusUI.label}
                                            </span>
                                        </td>
            ...
            {data && data.totalPages > 1 && (
                <div className="flex items-center justify-between px-6 py-4 bg-gray-50/50 border-t border-gray-100 text-sm z-0">
                    <span className="text-gray-500">
                        Hiển thị {data.number * data.size + 1} - {Math.min((data.number + 1) * data.size, data.totalElements)} trong tổng {data.totalElements} đợt
                    </span>
...
RegistrationTable.displayName = "RegistrationTable";