    ...
    return (
        <div className="relative flex justify-center" ref={menuRef}>
            ...
            {isOpen && (
                <div
                    className={clsx(
                        "fixed sm:absolute right-6 w-48 bg-white border border-gray-100 rounded-lg shadow-[0_4px_20px_-4px_rgba(0,0,0,0.1)] py-1 z-[9999]",
...
export const ClassTable: React.FC<ClassTableProps> = ({
    ...
    return (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden w-full flex flex-col relative">
                ...
                <table className="w-full text-sm text-left text-gray-500 table-auto min-w-[800px]">
                    <thead className="text-xs text-gray-700 uppercase bg-gray-50/70 border-b border-gray-100 font-bold">
                        <tr>
                            ...
                            <th className="px-5 py-4 w-[28%] max-w-[250px]">Môn học</th>
                            <th className="px-5 py-4 w-[12%]">Học kỳ</th>
                            <th className="px-5 py-4 w-[18%]">Giảng viên</th>
                            <th className="px-5 py-4 text-center w-[10%]">Sĩ số</th>
                            <th className="px-5 py-4 text-center w-[12%]">Trạng thái</th>
                            <th className="px-3 py-4 text-center w-[8%] sticky right-0 bg-gray-50/70 shadow-[-4px_0_6px_-2px_rgba(0,0,0,0.02)] z-20">Tác vụ</th>
                        </tr>
                    </thead>
                        ...
                        {isLoading ? (
                            skeletonRows.map((_, index) => (
                                <tr key={index} className="animate-pulse bg-white">
                                    ...
                                    <td className="px-5 py-4 max-w-[250px]"><div className="h-4 bg-gray-200 rounded w-3/4"></div></td>
                                    <td className="px-5 py-4"><div className="h-4 bg-gray-200 rounded w-full"></div></td>
                            ...
                            <tr>
                                <td colSpan={7} className="px-6 py-12 text-center bg-white">
                                    <div className="flex flex-col items-center justify-center gap-2 text-gray-400">
                                        ...
                                        <span className="text-sm">Vui lòng thay đổi bộ lọc hoặc thêm mới lớp học phần.</span>
                                    </div>
                                </td>
                            </tr>
                        ) : (
                            data.content.map((classItem, index) => {
                                ...
                                const statusUI = STATUS_UI_CONFIG[classItem.status];

                                return (
                                    <tr key={classItem.id} className="hover:bg-blue-50/30 bg-white transition-colors group relative">
                                        ...
                                        <td className="px-5 py-4 text-center whitespace-nowrap">
                                            <span className={clsx("font-semibold", isFull ? "text-red-600" : "text-gray-900")}>
                                                {classItem.currentStudents}
                                            </span>
                                            <span className="text-gray-400 text-xs mx-1">/</span>
                                            <span className="text-gray-500 text-sm">{classItem.maxStudents}</span>
                                        </td>
                                        <td className="px-5 py-4 text-center whitespace-nowrap">
                                            <span className={clsx("inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold tracking-wide border", statusUI?.style || "bg-gray-100 text-gray-700 border-gray-200")}>
                                                {statusUI?.label || classItem.status}
                                            </span>
                                        </td>
                                        <td className="px-5 py-4 text-center sticky right-0 bg-white group-hover:bg-blue-50/30 transition-colors shadow-[-4px_0_6px_-2px_rgba(0,0,0,0.02)] z-10 focus-within:z-50">
                                            <ActionMenu
                                                classItem={classItem}
                                                index={index}
            ...
            {data && data.totalPages > 1 && (
                <div className="flex items-center justify-between px-6 py-4 bg-gray-50/50 border-t border-gray-100 text-sm">
                    <span className="text-gray-500">
                        Hiển thị <span className="font-semibold text-gray-900">{data.number * data.size + 1}</span> đến <span className="font-semibold text-gray-900">{Math.min((data.number + 1) * data.size, data.totalElements)}</span> trong tổng <span className="font-semibold text-gray-900">{data.totalElements}</span> lớp
                    </span>
                        ...
                        {Array.from({ length: data.totalPages }).map((_, i) => (
                            <button
                                ...
                                className={clsx(
                                    ...
                                    data.number === i ? "z-10 text-blue-600 bg-blue-50 border-blue-300 font-medium" : "text-gray-500 bg-white border-gray-300 hover:bg-gray-100 hover:text-gray-700"
                                )}
                            >
                                {i + 1}
                            </button>
