...
// Toi uu: Su dung React.memo de co dinh danh sach, tranh re-render khi component cha bat tat Modal
export const ExamList: React.FC<ExamListProps> = memo(
    ({ exams, isLoading, onOpenExam, onCloseExam }) => {
        if (isLoading) {
            return (
            ...
            return (
                <div className="flex flex-col items-center justify-center py-20 bg-white rounded-xl border border-gray-200 shadow-sm border-dashed">
                    ...
                    <p className="text-sm text-gray-500 mt-1 text-center max-w-sm">
                        Lớp học này chưa có bài kiểm tra trắc nghiệm nào. Bấm
                        "Tạo bài kiểm tra" để bắt đầu.
                    </p>
                </div>
            );
        }
