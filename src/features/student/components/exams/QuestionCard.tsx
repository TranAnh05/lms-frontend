...
// Sử dụng React.memo để chặn re-render lãng phí khi các câu hỏi khác thay đổi
export const QuestionCard = React.memo<QuestionCardProps>(
    ({ question, index, selectedOptionId, onSelectOption }) => {
        return (
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 sm:p-8">
                {/* Tiêu đề và số thứ tự câu hỏi */}
                ...
                <div className="space-y-3 pl-0 sm:pl-13">
                    {question.options.map((option, optIdx) => {
                        ...
                        // Chuyển index thành ký tự chữ cái tương ứng (0 -> A, 1 -> B, ...)
                        ...
                        return (
                            ...
                            >
                                {/* Vòng tròn mô phỏng Radio Button */}
                                <div
                                    className={clsx(
                                ...
                                >
                                    {isSelected && (
                                        <div className="w-2.5 h-2.5 rounded-full bg-blue-600 animate-in zoom-in duration-200" />
                                    )}
                                </div>

                                {/* Nội dung chi tiết của đáp án */}
...
// Gán displayName hỗ trợ việc debug trên React DevTools dễ dàng hơn
